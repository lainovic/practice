...
class MaxHeap:
    def __init__(self,
                 capacity: int = 1,
                 greater_than_or_equal_predicate: Callable[[
                     T, T], bool] = None,
                 negative: Callable[[T], T] = None) -> None:
        """
        :note: in case of user-derived values, users must provide the callback for negating the values.
        Leaky abstractions, I know...
        """
        self.negation_op = negative if negative else lambda x: (-1) * x
        self.minheap = MinHeap(capacity,
                               less_than_predicate=None if greater_than_or_equal_predicate is None else
                               lambda x, y: not greater_than_or_equal_predicate(x, y))
